...
exports.extractFunctions = body => {
  ...
  const extractHelper = (funcs) => {
    ...
    throw Error('Could not extract with helper functions');
  }

  const oldExtractNTransform = () => {
  ...
  const extractNTransform = () => {
    try {
      functions.push(extractHelper([oldExtractNTransform, () => extractNTransformWithName(body)]));
    } catch (e) {
        console.warn(
